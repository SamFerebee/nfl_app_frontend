...
const CreateTeamForm = ({sendToHome, user, setUser}) => {
    ...
    const changeData = e => {
        switch (e.target.name){
            ...
            case "teamName":
                ...
                break;
            case "abbreviation":
                const tmp = {...formData, [e.target.name]: e.target.value};
                setFormData(tmp);
                break;
            case "quarterback":
    ...
    const genTeam = () =>{
        setFormData({
            logo: "https://i.pinimg.com/originals/d9/ea/51/d9ea5110919e111d19341805fcde88de.png",
            abbreviation: "ABC",
            teamName: "autoGenerated",
            user: user.id,
            quarterback: "Aaron Rodgers",
            runningback: "Saquon Barkley",
            wideout: "Julio Jones",
            lefttackle: "Trent Williams",
            righttackle: "Mitchell Schwartz",
            leftguard: "Quenton Nelson",
            center: "Jason Kelce",
            rightguard: "Brandon Scherff",
            tightend: "Evan Engram",
            leftend: "J.J. Watt",
            rightend: "Myles Garret",
            dtackle: "Aaron Donald",
            lolb: "Lorenzo Carter",
            rolb: "Tarrel Basham",
            mlb: "Blake Martinez",
            cb: "Stephon Gilmore",
    ...
    return (
        <div id="fullCreateForm">
            <button className="interiorButton" id="gen" onClick={genTeam}>Generate</button>
            <h2 id="remainingbudget">REMAINING BUDGET: ${salaryCap}</h2>
            <span id="createTeamName">Team name: <input type="text" name="teamName" id="teamNameField" onChange={changeData} value={formData.teamName} placeholder="Enter a team name"/> </span>
            <span id="createTeamAbbreviation">Abbreviation: <input type="text" name="abbreviation" id="abbreviationNameField" onChange={changeData} value={formData.abbreviation} placeholder="3 letters"/> </span>
            <span id="createTeamLogo">Logo: <input type="text" name="logo" id="logoField" onChange={changeData} value={formData.logo} placeholder="Link to a picture"/> </span>
            <form id="createTeamForm" onSubmit={handleSubmit}>
                ...
                <input id="submitNewTeam" type="submit" value="Create Team" className="aFormSubmit"/>
            </form>
